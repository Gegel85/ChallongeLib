...
namespace ChallongeAPI
{
	...
	private:
		...
		std::optional<std::pair<int, int>> _scores;
		std::optional<Time>                _startedAt;
		std::string                        _state;
		std::optional<unsigned>            _suggestedPlayOrder;
	...
	public:
		...
		const std::optional<size_t>              &getGroupId() const;
		...
		const std::optional<std::pair<int, int>> &getScores() const;
